...
if (process.env.NODE_ENV === 'test') {
  serverDb = new PouchDB('medicare', { skip_setup: true, adapter: 'memory' })
  localDb = new PouchDB('local_medicare', { skip_setup: true, adapter: 'memory' })
} else {
  serverDb = new PouchDB(`${process.env.REACT_APP_MEDICARE_API}/medicare`, {
    skip_setup: true,
  })

  localDb = new PouchDB('local_medicare', { skip_setup: true })
